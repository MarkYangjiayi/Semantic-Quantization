...
class FCN8VGG:
    ...
    def build(self, rgb, train=False, num_classes=151, random_init_fc8=False,
        ...
        with tf.name_scope('Processing'):
            ...
            bgr = tf.concat([
                (blue - VGG_MEAN[0])/63.75 - 1,
                (green - VGG_MEAN[1])/63.75 - 1,
                (red - VGG_MEAN[2])/63.75 - 1,
            ], 3)
            bgr = QA(bgr)
            if debug:
        ...
        self.conv1_1 = self._conv_layer(bgr, "conv1_1",quant=True)#1
        self.conv1_2 = self._conv_layer(self.conv1_1, "conv1_2")#2
        self.pool1 = self._max_pool(self.conv1_2, 'pool1', debug)

        self.conv2_1 = self._conv_layer(self.pool1, "conv2_1")#3
        self.conv2_2 = self._conv_layer(self.conv2_1, "conv2_2")#4
        self.pool2 = self._max_pool(self.conv2_2, 'pool2', debug)

        self.conv3_1 = self._conv_layer(self.pool2, "conv3_1")#5
        self.conv3_2 = self._conv_layer(self.conv3_1, "conv3_2")#6
        self.conv3_3 = self._conv_layer(self.conv3_2, "conv3_3")#7
        self.pool3 = self._max_pool(self.conv3_3, 'pool3', debug)

        self.conv4_1 = self._conv_layer(self.pool3, "conv4_1")#8
        self.conv4_2 = self._conv_layer(self.conv4_1, "conv4_2")#9
        self.conv4_3 = self._conv_layer(self.conv4_2, "conv4_3")#10
        ...
        self.conv5_1 = self._conv_layer(self.pool4, "conv5_1")#11
        self.conv5_2 = self._conv_layer(self.conv5_1, "conv5_2")#12
        self.conv5_3 = self._conv_layer(self.conv5_2, "conv5_3")#13
        if use_dilated:
            pad = [[0, 0], [0, 0]]
        ...
        self.fc6 = self._fc_layer(self.pool5, "fc6")#14

        if train:
            self.fc6 = tf.nn.dropout(self.fc6, 0.5)

        self.fc7 = self._fc_layer(self.fc6, "fc7")#15
        if train:
        ...
        self.upscore32 = self._upscore_layer(self.fuse_pool3,
                                             ...
                                             ksize=16, stride=8,quant=True)#21
    ...
    def _conv_layer(self, bottom, name,quant=True):
        with tf.variable_scope(name) as scope:
            filt = self.get_conv_filter(name)
            bias = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')
            ...
            if quant:
                relu = QE(relu)#<--------------------------------------------
                _activation_summary(relu,name,'activation')
                relu_q = QA(relu)#<--------------------------------------------
            return relu_q
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            _activation_summary(bias,name,'activation')

            bias = QE(bias)#<--------------------------------------------
            bias = QA(bias)#<--------------------------------------------
            _activation_summary(bias,name,'activation_Q')
            if debug:
    ...
    def _score_layer(self, bottom, name, num_classes):
        with tf.variable_scope(name) as scope:
            ...
            weights = self._variable_with_weight_decay(shape, stddev, w_decay, name,
                                                       decoder=True)
            bias = tf.nn.conv2d(bottom, weights, [1, 1, 1, 1], padding='SAME')
            # Apply bias
    ...
    def _upscore_layer(self, bottom, shape,
        ...
        with tf.variable_scope(name):
            ...
            weights = self.get_deconv_filter(f_shape,quant)
            self._add_wd_and_summary(weights, self.wd, "fc_wlosses")
            deconv = tf.nn.conv2d_transpose(bottom, weights, output_shape,
                                            strides=strides, padding='SAME')
            _activation_summary(deconv,name,'activation')
            if quant:
                deconv = QE(deconv)#<--------------------------------------------
                deconv = QA(deconv)#<--------------------------------------------
            ...
            _activation_summary(deconv,name,'activation_Q')
        return deconv

    def get_deconv_filter(self, f_shape,quant=True):
        width = f_shape[0]
        height = f_shape[1]
        f = ceil(width/2.0)
        c = (2 * f - 1 - f % 2) / (2.0 * f)
        ...
        if quant:
            self.W.append(var)
            self.W_q_op.append(tf.assign(var,Qinit(var,bitsU)))
            self.W_clip_op.append(tf.assign(var,clip(var,bitsW)))
            var = QW(var)#<--------------------------------------------
        return var
    ...
    def get_conv_filter(self, name):
        ...
        _activation_summary(var,name,'weight')
        self.W.append(var)
        self.W_q_op.append(tf.assign(var,Qinit(var,bitsU)))
        self.W_clip_op.append(tf.assign(var,clip(var,bitsW)))
        var = QW(var)#<--------------------------------------------
        ...
        _activation_summary(var,name,'weight_Q')
        return var
...
def _activation_summary(x,tensor_name,summary_type):
...
# def _activation_summary(var,var1,var2):
#     return var
