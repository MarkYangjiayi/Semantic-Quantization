...
class NN(object):
  ...
  def _VGG7(self):
    ...
    with tf.variable_scope('FC'):
      with tf.variable_scope('C0'):
        x = self._conv(x, 7, 4096)
        x = self._activation(x)
      with tf.variable_scope('C1'):
        x = self._conv(x, 1, 4096)
        x = self._activation(x)
      with tf.variable_scope('C2'):
        x = self._conv(x, 1, 151)
        x = self._activation(x)

    print(tf.shape(x))
    with tf.variable_scope('US'):
        x = self._upsample(x,224)
  ...
  def _loss(self, out, labels):
    ...
    with tf.name_scope('error'):
      ...
      # in_top_k = 1
      error = tf.reduce_mean(tf.cast(tf.logical_not(tf.math.equal(out, labels)), tf.float32))
  ...
  def _upsample(self, x, shape):
    ...
    x = tf.image.resize_bilinear(x,[shape,shape],True)
